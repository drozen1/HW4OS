...
const int BLOCK_MAX_COUNT=10, BLOCK_MAX_SIZE=10, LAST=99;
...
void test_malloc_then_free() {
	...
	const size_t BLOCK_SIZES[BLOCK_MAX_COUNT] = {3, 8, 1, 7, 99};
	...
	const byte DATA[BLOCK_MAX_COUNT][BLOCK_MAX_SIZE] =
			{{1,2,3}, {11,12,13,14,15,16,17,18}, {20}, {1,2,3,4,5,6,7}, {99}};
...
void test_calloc() {
	const size_t BLOCK_SIZES[BLOCK_MAX_COUNT] = {1,2,3, 99};
	byte DATA[BLOCK_MAX_COUNT][BLOCK_MAX_SIZE] =
			{{10}, {20,21}, {30,31,32}, {99}};
	const byte ZEROS[BLOCK_MAX_COUNT][BLOCK_MAX_SIZE] = {{},{},{},{99}};
	byte *heap = static_cast<byte*>(sbrk(0));
	HeapState initial;
	get_initial_state(initial);
...
void test_realloc() {
	const size_t BLOCK_SIZES[BLOCK_MAX_COUNT] = {3,4,5, 99};
	byte DATA[BLOCK_MAX_COUNT][BLOCK_MAX_SIZE] =
			{{1,2,3}, {1,2,3,4}, {11,12,13,14,15}, {99}};
	byte *heap = static_cast<byte*>(sbrk(0));
	HeapState initial;
	get_initial_state(initial);
